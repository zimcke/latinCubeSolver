...
checkAfter(Solution):-
	formatSolution(Solution,SolutionF),
	ones(Ones),
	w_constraint(SolutionF,Ones),
	h_constraint(SolutionF,Ones),
	d_constraint(SolutionF,Ones).
...
getWConstraints([d(I,J)|Other],[d(I,J)|Other],Ones,Indices,Result):-
			sort(Indices,IndicesSorted),
			convert(IndicesSorted,1,Ones,[],Result).
...
getHConstraints([],[],Ones,Indices,Result):-
			sort(Indices,IndicesSorted),
			convert(IndicesSorted,1,Ones,[],Result).
